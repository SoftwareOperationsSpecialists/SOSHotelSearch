...
public class MapDriver {
  ...
  private double radiusInMiles = 50; // defines radius (in miles) to display hotels from searched for
  private double radiusInDegrees = radiusInMiles / DEGREES_TO_MILES;
    ...
    mapView.setOnMapReadyHandler(new MapReadyHandler() {
      ...
      public void onMapReady(MapStatus status) {
        if (status == MapStatus.MAP_STATUS_OK) {
          ...
          mapView.getServices().getGeocoder().geocode(request, new GeocoderCallback(map) {
            ...
            public void onComplete(GeocoderResult[] result, GeocoderStatus status) {
              if (status == GeocoderStatus.OK) {
                ...
                setError("ERROR: There was an error connecting to the Google Servers.");
                setErrorStatus(true);
              }
              else if (status == GeocoderStatus.INVALID_REQUEST) {
                setError("ERROR: This request was invalid.");
                setErrorStatus(true);
              }
              else if (status == GeocoderStatus.OVER_QUERY_LIMIT) {
                setError("ERROR: The web page has gone over the requests limit in too short a period of time.");
                setErrorStatus(true);
              }
                ...
                setError("ERROR: Your request could not be processed due to a server error.");
                setErrorStatus(true);
              }
              else {
                setError("ERROR: No results found for this location.");
                setErrorStatus(true);
              }
            }
          });
          ...
          setError("ERROR: Map status error.");
          setErrorStatus(true);
        }
      }
    });
    try {
      TimeUnit.SECONDS.sleep(2); // wait 2 seconds to finish parsing the text file
